...
| `/` | 仪表盘 | 全站 PV 趋势、各站点聚合概览 |
| `/sites` | 网站管理 | 站点列表、新增站点 |
| `/sites/:id` | 网站详情 | 趋势图 / 页面列表 / 访客分析 / 设备分析 / 阅读分析 / 访问日志 / Token 管理 |
...
| 模块 | 核心指标 |
|---|---|
| 统计卡片 | 总 PV、今日 PV、总页数、今日 UV |
| 趋势图 | 7 / 30 / 90 / 365 天折线图（PV） |
| 热门页面 | Top 10，含 PV 占比 |
| 页面列表 | 全量页面，支持搜索 / 排序 / 分页 |
| 访客概览 | UV 趋势、今日 / 周期内独立访客数 |
| 设备分析 | 设备类型 / 浏览器 / 操作系统分布饼图 |
| 阅读分析 | 25/50/75/100% 漏斗图、平均阅读深度、完成率 |
| 访问日志 | 原始日志表格，按路径 / 日期范围筛选 |
| Token 管理 | 创建 / 停用 / 删除 Token |
