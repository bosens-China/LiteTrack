...
const sites: FastifyPluginAsync = async (fastify, _opts): Promise<void> => {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      const { userId } = request.user as UserPayload;
      const siteId = parseId((request.params as SiteParams).id);
      if (!siteId) return reply.code(400).send({ error: '无效的 ID' });
   ...
   */
  fastify.patch(
    '/:siteId/tokens/:tokenId',
    { onRequest: [fastify.authenticate] },
    async (request, reply) => {
      const { userId } = request.user as UserPayload;
      const siteId = parseId((request.params as TokenParams).siteId);
      const tokenId = parseId((request.params as TokenParams).tokenId);
      if (!siteId || !tokenId)
